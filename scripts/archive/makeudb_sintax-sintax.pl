...
my $db_path = "$txn_dir/sintax.udb"; # used only in the next step
$cmd = "usearch -makeudb_sintax $db_file -output $db_path";
system ( $cmd );

### step 2
$q_file = "$out_dir/$prj_name$prev_step_prms.fac"; # sic!
$out_file = "$out_dir/$prj_name--$prms[1]--$conf_level.sintax";
$cmd = "usearch -sintax $q_file -db $db_path -tabbedout $out_file -strand both -sintax_cutoff $conf_level";
print "cmd: $cmd\n";
system ( $cmd );
